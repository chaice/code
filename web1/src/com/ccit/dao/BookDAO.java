...
public class BookDAO {
	...
	public boolean addBook(Book b){
		...
		int i = db.getUpdate(sql, b.getCode(),b.getTitle(),b.getAuthor(),b.getPublishing(),b.getTotal(),b.getCount());
		return i==1;
	}
	public boolean alterBook(Book b){
		String sql = "update `book` set `code`=?,`author`=?,`publishing`=?,`total`=?,`count`=? where`title`=?";
		int i = db.getUpdate(sql, b.getCode(),b.getAuthor(),b.getPublishing(),b.getTotal(),b.getCount(),b.getTitle());
		return i==1;
	}
	public boolean deleteBook(String title){
		String sql = "delete from `book` where `title`=?";
		int i = db.getUpdate(sql, title);
		return i==1;
	}
	...
	public List<Book> findBooks(String str){
		String sql ="select  `id`,`code`,`title`,`author`,`publishing`,`total`,`count` from`book`where `title`like '%str% '";
		return db.getQueryAll(sql, b, str);
	}
